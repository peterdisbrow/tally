/**
 * Equipment Tester — connection probes for AV gear
 *
 * Extracted from main.js. Tests reachability of ATEM, OBS, Companion,
 * HyperDeck, PTZ, ProPresenter, Dante, vMix, Resolume, Mixer, Encoder, etc.
 */
...
/**
 * Test equipment connection by type.
 *
 * @param {object} params - { type, ip, port, password, url, protocol, mixerType, encoderType, statusUrl }
 * @param {object} deps - injected dependencies: { tryTcpConnect, tryHttpGet }
 *   These come from networkScanner.js and are passed in by main.js.
 * @returns {Promise<{ success: boolean, details: string }>}
 */
async function testEquipmentConnection(params, deps) {
  const { tryTcpConnect, tryHttpGet } = deps;
