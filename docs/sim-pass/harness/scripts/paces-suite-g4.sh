#!/usr/bin/env bash
# Gate on 6fa6c97 (audio: X32 + SQ). Usage: paces-suite-g3.sh <pass-tag A|B>
# Local relay :3400 + MockLab :9910/:9911 only. Sequential (shared :2 display + relay).
set -u
P=${1:-A}
cd /workspace/tally-linux
export DISPLAY=:2
AI=/tmp/g4-appimage/squashfs-root/tally-connect
DEB=/tmp/g4-deb/opt/Tally/tally-connect
run() { # id, script, [packaged bin], [relay mode]
  local id=g4$P-$1 script=$2 bin=${3:-} mode=${4:-stop}
...
run appimage-sq sim/booth-sq.mjs $AI
...
run deb-sq sim/booth-sq.mjs $DEB
bash scripts/cleanup-tally.sh
R=results; T=g4$P
