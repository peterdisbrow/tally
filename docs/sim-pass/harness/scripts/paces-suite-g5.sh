#!/usr/bin/env bash
# Milestone gate g5 on 1d78676 (dLive/Avantis + Yamaha CL/QL/TF + ProPresenter, on top of X32/SQ/OBS).
# Usage: paces-suite-g5.sh <pass-tag A|B>. Local relay :3400 + MockLab :9910/:9911 only.
# Sequential (shared :2 display + relay). Packages from the fresh 1d78676 build.
set -u
P=${1:-A}
cd /workspace/tally-linux
export DISPLAY=:2
AI=/tmp/g5-appimage/squashfs-root/tally-connect
DEB=/tmp/g5-deb/opt/Tally/tally-connect
run() { # id, script, [packaged bin], [relay mode]
  local id=g5$P-$1 script=$2 bin=${3:-} mode=${4:-stop}
