...
/**
 * Chat API route handlers (church-facing + admin-facing).
 *
 * @param {import('express').Express} app
 * @param {Object} ctx
 * @param {Object} ctx.db
 * @param {Function} ctx.requireChurchAppAuth
 * @param {Function} ctx.requireAdmin
 * @param {Object} ctx.chatEngine
 * @param {Function} ctx.handleChatCommandMessage
 * @param {Function} ctx.log
 */
module.exports = function setupChatRoutes(app, ctx) {
  const { db, requireChurchAppAuth, requireAdmin, chatEngine, handleChatCommandMessage, log } = ctx;
