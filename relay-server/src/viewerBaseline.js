...
class ViewerBaseline {
  ...
  compute(churchId, serviceDay, opts = {}) {
    ...
    // Cache to DB
    this._cacheBaseline(churchId, serviceDay, result, opts.instanceName, opts.roomId);
  ...
  getBaseline(churchId, serviceDay) {
    ...
    // Recompute
    return this.compute(churchId, serviceDay);
  }

  _cacheBaseline(churchId, serviceDay, result, instanceName, roomId) {
    try {
  ...
  recomputeAll() {
    try {
      const churches = this.db.prepare('SELECT churchId, service_times FROM churches').all();
      let count = 0;
